...
class Calendar(CTkToplevel):
    def __init__(self, session, dics, dics2=None, prac='', **kwargs):
        ...
        self.date_lesson = set([x['date'] for x in session.student_rows(dics['id_group'], dics['subject_id'], prac=prac)[
                                   'rows'][0]['lessons']])
    ...
    def open_frame(self):
        if self.prac == '':
            StudentFrame(self.session, dics=self.disc, dics2=self.disc2, date_from=self.date.get(), date_whis=self.date.get())
        else:
