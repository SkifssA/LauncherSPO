...
class Calendar(CTkFrame):
    def __init__(self, master, session, dics, dics2=None, prac='', **kwargs):
        ...
        self.date_lesson = set(
            [x['date'] for x in session.student_rows(dics['id_group'], dics['subject_id'], prac=prac)[
                'rows'][0]['lessons']])
    ...
    def open_frame(self):
        if self.prac == '':
            ...
            if self.disc2 is not None:
                frane_s = StudentFrame(self.master1, self.session, dics=self.disc, dics2=self.disc2, prac=self.prac, date_from=self.date.get(),
                             date_whis=self.date.get())
            else:
                frane_s = StudentFrame(self.master1, self.session, dics=self.disc, prac=self.prac, date_from=self.date.get(),
                             date_whis=self.date.get())
        frane_s.root.grid()
        self.grid_forget()
