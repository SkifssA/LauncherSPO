...
class APP(CTk):
    ...
    def create_student_frame(self):
        ...
        if len(tr) + len(pr) == 1:
            if len(tr) == 1:
                self.studen_frame = StudentFrame(self.session, ListOfDisciplines.Theory[tr[0]], '', width=1280,
                                                 height=500)
                self.studen_frame.add_v_group(ListOfDisciplines.Theory[tr[0]+1])
            elif len(pr) == 1:
                self.studen_frame = StudentFrame(self.session, ListOfDisciplines.Practice[pr[0]], '1', width=1280,
                                                 height=500)
                if ListOfDisciplines.Practice[pr[0]]['name'].find('/2') != -1:
                    self.studen_frame.add_v_group(ListOfDisciplines.Practice[pr[0] + 1])
            self.studen_frame.grid(row=0, column=0, pady=10, padx=10, columnspan=3)
        else:
