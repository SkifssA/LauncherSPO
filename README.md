...
Заполнение электронного журнала темами, закрытие, открытие занятий, выставление оценок и посещяемости.

За основу взят [Eljurnal](https://github.com/ChePchik/Eljurnal)

## Установка для разработчиков

1. Скачиваем и устанавливаем [Python 3.10](https://www.python.org/downloads/release/python-3109/) (лучше).
2. Скачать проект любым способом. Перейдите в папку проекта, главное не архив.
3. Откройте командную строку (терминал) в папке проекта. В windows зажать shift и выбрать открыть консоль\терминал\\...
4. Введите следующую команду для установки необходимых зависимостей: `pip install -r requirements.txt`, если пишет, что `pip` не установлен. Написать эту команду `python -m pip install -r requirements.txt`
...
Запускаем или `GUI.py`

Вам будет предложено окно авторизации.

![](img/img1.png)

_запуск изменился._ Теперь от вас потребуется вести не логин и пароль, а токены авторизации. Как их получить:

1. Авторизуемся на сайте журнала, как обычно.
2. Далее открываем инструменты разработчика в браузере, в зависимости от бразуера они могут открываться по разному. `F12`, либо правой кнопкой мыши по странице - исследовать\\...
3. Открыть вкладку сеть. Вот как это выглядит.

![](img/img13.png)

4. На странице открыть `Журнал теоретических занятий`
5. Во вкладке `сеть`, в `инструментах разработчика`, появится много запросов.
6. Выбираем самый последний который есть. В нём выбираем вкладку `Cookie\Куки`. В неё нам понадобятся только 2 поля. `csrftoken` и `ssuz_sessionid`

![](img/img14.png)

`csrftoken` - вставляем в 1 поле и `ssuz_sessionid` - вставляем во 2 поле, в приложении.
...
Вставлять текст можно, но только на английской раскладке.
...
- Кто хочет поэксперементировать, есть zurnal.ipynb
